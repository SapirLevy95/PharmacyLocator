  ...
  if (userName) {
    if (await isUsernameExists(userName)) {
      ...
      if (user.passowrd === passowrd) {
        console.log(
          `Logged in successfully user=${user.userName} location=${user.locationId}`
        );
        return user;
      } else {
        console.log(
          `Wrong password entered=${passowrd} original=${user.passowrd}`
        );
        return null;
      }
    }
...
export async function getPharmacyFromDB(pharmacyId) {
  ...
  const usersInPharmacy = users.filter(
    (user) => user.doc.locationId === pharmacyId
  );
